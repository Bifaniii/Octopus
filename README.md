...
O escopo completo, regras de negócio (RN-01..RN-08), squad e cronograma das 5 sprints estão no `TAP.md` da branch `main`.

## Microsserviços

**Cada microsserviço vive na sua própria branch**, com `Dockerfile`, `docker-compose.yml` e `.env.example`
próprios — sobe sozinho, sem depender dos outros. Esta branch contém o **microsserviço de medicamentos**.
...
Tudo abaixo roda de dentro de `octopus-msmedications/`.
...
| Endpoint | O que faz |
| :------- | :-------- |
| `GET /api/medicacoes` | Lista tudo, ou filtra com `?fabricante=` / `?nomeComercial=` |
...
- `Medicacao` com `UUID`, **tipo de esquema** (`CONTINUO`/`SINTOMATICO`) e **interações proibidas** simétricas
  entre medicamentos — os dois itens do escopo do TAP para esta tela.
- Spring Security + JWT validando o token do `octopus-msusuario`; erros padronizados por
  `GlobalExceptionHandler` (400, 401, 403, 404, 409, 422).
- Flyway com `V1__medicacoes.sql` e tabela de histórico própria; 11 testes automatizados.
...
- [ ] Sprint 3/4: microsserviço de aplicação de doses, separado deste.
- [ ] Avaliar se o item de prescrição terá `tipoEsquema` próprio, nascendo com o valor do catálogo.
